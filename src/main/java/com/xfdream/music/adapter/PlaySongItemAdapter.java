...
public class PlaySongItemAdapter extends BaseAdapter {
	 ...
	 * */
	public PlaySongItemAdapter(Context context, List<String[]> data, int[] playerInfo){
		this.context=context;
		this.data=data;
		this.playerInfo=playerInfo;
	}
	
	public PlaySongItemAdapter setItemListener(ItemListener mItemListener){
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			convertView=LayoutInflater.from(context).inflate(R.layout.play_song_list_item, null);
		...
		if(Integer.valueOf(d[0])==playerInfo[0]){
			...
			if(playerInfo[1]==MediaPlayerManager.STATE_PAUSE){
				...
				EventBus.getDefault().post(new CommonMessage(Constants.MSG_PLAYA_OVER,null));
			}
		}else{
			viewHolder.tv_song_list_item_number.setText((position+1)+"");
			viewHolder.tv_song_list_item_number.setBackgroundResource(0);
