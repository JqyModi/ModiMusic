...
import com.xfdream.music.custom.XfDialog.Builder;
...
public class Common {
	 ...
	 * @return
     */
	public static int getCurrentIndex(int currentTime) {
		if (currentTime>0){
			for (int i=0;i<LRC_WORD_DATA.size();i++){
				boolean lInTime = LRC_WORD_DATA.get(i).isInTime(currentTime);
				if (LRC_WORD_DATA.get(i).getFromTime() == currentTime){
					return i;
				}else if (lInTime){
					return i;
				}
	...
	public static int getLrcIndex(long currentTime,long duration,List<Sentence> lrcLists) {
		int index = 0;
		int middleTime = 0;
		int middleTime1 = 0;
		if (currentTime < duration) {
			for (int i = 0; i < lrcLists.size(); i++) {
				Sentence lSentence = lrcLists.get(i);
				Sentence lSentence1 = lrcLists.get(i+1);
				middleTime = (int) ((lSentence.getToTime()-lSentence.getFromTime())/2);
				middleTime1 = (int) ((lSentence1.getToTime()-lSentence1.getFromTime())/2);
				int lI = (int) (lSentence.getFromTime() + middleTime);
				int lI1 = (int) (lSentence1.getFromTime() + middleTime);
				if (i < lrcLists.size() - 1) {
					if (currentTime < lI && i == 0) {
						index = i;
					}
					if ((currentTime > lI) && currentTime < lI1) {
						index = i;
					}
				}
				if ((i == lrcLists.size() - 1) && currentTime > lI) {
					index = i;
				}
			}
		}
	 ...
	 * @return
     */
	public static Bitmap getBackgroundByPath(String path){
		//根据路径获取图片
